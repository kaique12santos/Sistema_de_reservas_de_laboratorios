    ...
    <Dialog open={open} onClose={onCancel} PaperProps={{ sx: { borderRadius: 3, p: 1 } }}>
      ...
      <DialogContent>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
          O ciclo atual será desativado e este passará a ser o vigente. Todas as novas reservas seguirão este calendário.
        </Typography>
      </DialogContent>
...
export default function AcademicCyclesPage() {
  ...
  // 🚀 O NOSSO NOVO MOTOR ACIONADO PELO FRONTEND
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      setNotify({ open: true, message: `Ciclo ${activateDialog.cycle.name} ativado com sucesso!`, severity: 'success' });
      await load();
    } catch (err) {
      setNotify({ open: true, message: err?.response?.data?.error || 'Erro ao ativar ciclo.', severity: 'error' });
    } finally {
      setActionLoading(false);
    }
  };
          ...
          <Table sx={{ minWidth: 650 }}>
            <TableHead sx={{ bgcolor: '#fafafa' }}>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 'bold' }}>Limite Coordenação</TableCell>
              ...
              {loading ? (
                ...
                cycles.map((cycle, index) => (
                  ...
                  >
                    ...
                    <TableCell>{formatDate(cycle.admin_exclusive_end_date)}</TableCell>
                    <TableCell>
                      <Chip
                        label={cycle.is_active ? 'Vigente' : 'Inativo'}
                        size="small"
